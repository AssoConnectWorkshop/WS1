...
| D2 | Sites orphelins : `Site.numcli` pointe vers un client absent de `Client`. | Rattachés à un client sentinelle « Client inconnu (migration) » (`legacy_id = -1`), d'où `clients` = 2 324 lignes pour 2 323 en source. Liste dans le rapport (`sites_orphelins`). | Réaffecter chaque site au bon client ou l'archiver (`supprime_le`). Supprimer ensuite le client sentinelle. |
| D3 | Interventions orphelines : `Intervention.cptsit` sans site correspondant. | Ignorées (`transform` → `null`), listées dans le rapport (`interventions_orphelines`). | Décider : recréer le site, ou accepter la perte (souvent des sites supprimés). Chiffre à relever dans le rapport. |
...
1. Après le transfert complet, copier ici les compteurs du rapport : orphelins (D2, D3), codes créés (D4), écarts source/cible par table.
