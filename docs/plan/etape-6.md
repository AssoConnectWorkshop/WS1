...
- [x] 6.8 Documents vers Storage : `scripts/migrate-documents/` (Node, à exécuter sur le PC de l'utilisateur avec `SUPABASE_URL` et `SUPABASE_SERVICE_ROLE_KEY`) : parcourt `interventions.chemin_bon_pdf` et `devis.fichier_chemin` (format hyperlien Access `libellé#\\serveur\chemin#` ou chemin brut), copie dans les buckets privés `bons` et `devis-documents` sous `legacy/<id>.<ext>`, remplace la colonne par le chemin Storage ; options `--dry-run`, `--limit`, `--table` ; rapport `last-run-report.md` (introuvables, ignorés, erreurs). `sites.dossier_chemin` désigne des dossiers : comptés, non copiés. Non exécutable depuis l'environnement de développement (pas d'accès au serveur de fichiers).
- [ ] Reste à faire par l'utilisateur : variables Vercel `RESEND_API_KEY`, `EMAIL_FROM`, `CRON_SECRET` ; paramètres d'application (e-mail SAV, SIRET, n° d'attestation) ; logos dans le bucket `logos` (`fmc.png`, `donneur-<id>.png`) ; exécution du script 6.8. → Repris et détaillé dans `docs/plan/etape-7.md`. En attendant, l'application fonctionne sans : e-mails simulés et journalisés, cron inactif, PDF sans logo, anciens chemins affichés tels quels.
