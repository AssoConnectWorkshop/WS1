...
- [x] Revue transverse en fin d'étape : composants partagés (`Messages` pour les bandeaux d'URL, `Champ` / `Case` / `Section` pour les formulaires, `Chemin`, `RechercheSite`, `Contacts`), helpers zod (`lib/zod-form` : `texte`, `nombre`, `entier`, `booleen`, `premiereErreur`), `lib/action-utils` (`requireUtilisateur`, `requireAdministrateur`, `redirectWithError`), `formatNom`. Pas de scénarios `docs/plan/tests/etape-5-*.md` : l'accès Supabase n'est pas disponible dans l'environnement de développement ; la validation se fait sur la Preview Vercel (build + migrations) et par l'utilisateur en production.
